...
export default {
    ...
    async getUserRegistration(userId: string, eventId: string): Promise<UserEventDTOS | null> {
        console.log(`[userEventRepository.getUserRegistration] Recebido - userId: ${userId}, eventId: ${eventId}`);
    ...
    async updateStatusForUsers(userIds: string[], eventId: string, status: number): Promise<void> {
        await client.userEvent.updateMany({ where: { userId: { in: userIds }, eventId }, data: { status } });
    },
